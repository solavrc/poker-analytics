...
with
    action_events as (
        select
            ...
            case
                when value:"ActionType"::number = 2
                then 'FOLD'
                when value:"ActionType"::number = 0
                then 'CHECK'
                when value:"ActionType"::number = 3
                then 'CALL'
                when value:"ActionType"::number = 1
                then 'BET'
                when value:"ActionType"::number = 4
                then 'RAISE'
                when value:"ActionType"::number = 5
                then 'ALL_IN'
            end as action_name,
    ...
    last_actions as (select hand_id, event_timestamp from best_actions where next_action_seat = -2),
    ...
    previous_phases as (select a.hand_id, a.event_timestamp, lag(a.phase) over (partition by a.hand_id order by a.event_timestamp) as previous_phase from best_actions a),
    ...
    final_actions as (
        select
            ...
            a.event_timestamp,
            a.seat_index,
            case when l.hand_id is not null then coalesce(p.previous_phase, 0) else a.phase end as phase,
            a.action_name,
            coalesce(a.player_chip, 0) as player_chip,
            coalesce(a.bet_chip, 0) as bet_chip,
            coalesce(a.pot, 0) as pot,
            row_number() over (
                partition by a.hand_id, case when l.hand_id is not null then coalesce(p.previous_phase, 0) else a.phase end order by a.event_timestamp
            ) as action_order,
            get(h.seat_user_ids, a.seat_index) as player_id,
            h.bb,
            a.sender_user_id as source_user_id  -- 情報の提供元を記録
        from best_actions a
        left join {{ ref("stg_hands") }} h on a.hand_id = h.hand_id
        left join last_actions l on a.hand_id = l.hand_id and a.event_timestamp = l.event_timestamp
        left join previous_phases p on a.hand_id = p.hand_id and a.event_timestamp = p.event_timestamp
    )
...
-- - チップ情報（bet_chip, pot, player_chip, bb）
-- - データソース情報（source_user_id）
select hand_id, event_timestamp, phase, action_order, player_id, action_name, bet_chip, pot, player_chip, seat_index, bb, source_user_id
from final_actions
