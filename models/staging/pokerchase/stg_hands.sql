...
with
    deal_events as (
        select
            ...
            case when value:"Game":"NextBlindUnixSeconds"::number = -1 then 'RING_GAME' else 'TOURNAMENT' end as game_type,
    ...
    -- - 終了時刻を記録
    hand_results as (
        select hand_id, sender_user_id, event_timestamp as end_timestamp
        from {{ ref("stg_hand_events") }}
        where api_type_id = 306  -- EVT_HAND_RESULTS
        qualify row_number() over (partition by hand_id order by event_timestamp desc) = 1
    ),
    ...
    -- - 最新のイベントを選択
    flop_cards as (
        select hand_id, sender_user_id, value:"CommunityCards" as cards, value:"Progress":"Phase"::number as phase
        from {{ ref("stg_hand_events") }}
        where
            api_type_id = 305  -- EVT_DEAL_ROUND
    ...
    -- - 最新のイベントを選択
    result_cards as (
        select hand_id, sender_user_id, value:"CommunityCards" as cards
        from {{ ref("stg_hand_events") }}
        where
            api_type_id = 306  -- EVT_HAND_RESULTS
            and value:"CommunityCards" is not null
    ...
    best_cards as (
        select hand_id, cards, sender_user_id, 'FLOP' as phase
        from flop_cards
        union all
        select hand_id, cards, sender_user_id, 'TURN' as phase
        from turn_cards
        union all
        select hand_id, cards, sender_user_id, 'RIVER' as phase
        from river_cards
        union all
        select hand_id, cards, sender_user_id, 'RESULT' as phase
        from result_cards
        qualify
            row_number() over (
    ...
    combined_cards as (
        select distinct
            ...
            coalesce(
                -- 通常のフェーズ進行の場合
                case
                    when array_size(array_cat(array_cat(coalesce(f.cards, array_construct()), coalesce(t.cards, array_construct())), coalesce(r.cards, array_construct()))) > 0
                    then array_cat(array_cat(coalesce(f.cards, array_construct()), coalesce(t.cards, array_construct())), coalesce(r.cards, array_construct()))
                    -- プリフロップオールインの場合
                    else res.cards
                end,
                array_construct()
        ...
        left join best_cards f on d.hand_id = f.hand_id and f.phase = 'FLOP'
        left join best_cards t on d.hand_id = t.hand_id and t.phase = 'TURN'
        left join best_cards r on d.hand_id = r.hand_id and r.phase = 'RIVER'
        left join best_cards res on d.hand_id = res.hand_id and res.phase = 'RESULT'
    ),
    ...
    -- - hand_idごとにグループ化
    card_strings as (
        select c.hand_id, c.community_cards, {{ convert_card_array_to_strings("value") }} as community_cards_str
...
from best_deal_events d
left join card_strings c on d.hand_id = c.hand_id
left join hand_results hr on d.hand_id = hr.hand_id
